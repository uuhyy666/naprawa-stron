// Szkielet: Cloudtalk (webhook) -> Notion (API) — dowód koncepcji dla zlecenia 143981
// Użycie: node webhook2notion.js  (nasłuchuje na http://127.0.0.1:9970/cloudtalk)
...
function mapCloudtalkToNotion(c) {
  return {
...
http.createServer((req, res) => {
  if (req.method === 'POST' && req.url.startsWith('/cloudtalk')) {
    let b = '';
    req.on('data', c => b += c);
    req.on('end', () => {
      try {
        const c = JSON.parse(b);
        console.log('Webhook Cloudtalk:', c.event || '(brak pola event)');
        notionCreate(mapCloudtalkToNotion(c));
        res.writeHead(200); res.end('OK');
      } catch (e) { res.writeHead(400); res.end('bad json'); }
    });
  } else { res.writeHead(404); res.end(); }
}).listen(9970, () => console.log('Nasłuch webhooka Cloudtalk na :9970/cloudtalk — test: curl -X POST localhost:9970/cloudtalk -d \'{"contact":{"name":"Jan Kowalski"},"agent":"Adam","duration":120}\''));
